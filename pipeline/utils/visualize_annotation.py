...
Author: chat.openai.com/o3 - updated 2025-07-17
"""
...
ARROW_SCALE_PX_PER_CM = 5  # 1 cm motion = 5 pixels arrow length
FONT = 1
...
def visualize_movements(
    ...
    with (
        imageio.get_writer(out_path, fps=fps, codec="libx264", quality=8) as writer,
...
def load_frames_from_dir(dir_path: str) -> List[np.ndarray]:
    ...
    paths = sorted(glob(os.path.join(dir_path, "*")))
    if not paths:
...
def main(argv: List[str] = None) -> None:
    ...
    visualise_movements(frames, T_cam_ee, args.out, args.fps)
